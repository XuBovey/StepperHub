...
void PrintStepperStatusStr(stepper_status status) {
  ...
  if (status & SS_BREAKING) {
    kprintf("%sBREAKING", separator);
    separator = " | ";
  }
  if (status & SS_BREAKCORRECTION) {
    kprintf("%sBREAKCORRECTION", separator);
    separator = " | ";
  }
  if (status & SS_STARTING) {
    kprintf("%sSTARTING", separator);
    separator = " | ";
  }
  if (status & SS_RUNNING_BACKWARD) {
    kprintf("%sRUNNING_BACKWARD", separator);
    separator = " | ";
  }
  if (status & SS_RUNNING_FORWARD) {
    kprintf("%sRUNNING_FORWARD", separator);
  }
  if (status == 0x00) {
    kprintf("UNDEFINED");
  }
  ...
  if (stepper == '\0') {
    ...
    switch (command) {
      ...
      default:
        ...
        kprintf("ERROR -  Program error in commands decoder.");
        break;
    }
  }
    ...
    if (parameter == PARAM_ALL) {
        ...
        do {
            ...
            if (param == PARAM_STATUS) {
                kprintf("\t.%s = 0x%.2lX ", request_params_arry[param], (int32_t)value);
        ...
        if (parameter == PARAM_STATUS) {
            kprintf(".%s = 0x%lX2 ", request_params_arry[parameter], (int32_t)value);
            PrintStepperStatusStr((stepper_status)value);
            kprintf("\r\n");
        } else {
