...
void SetAccelerationByMinSPS(stepper_state * stepper) {
    ...
    // Translating the floating point acczeleration value into "Prescaller + discreete AccSPS"
...
stepper_error Stepper_SetupPeripherals(
              char stepperName, 
...
void stopStepper(stepper_state * stepper){
  if (stepper == NULL)
    return;
  stepper->status = SS_STOPPED;
...
void Stepper_LoadConfig(void) {
  int32_t * configPtr = (int32_t *)ADDR_FLASH_SECTOR_11;
  int32_t i = 0;
...
void Stepper_SaveConfig(void) {
  uint32_t configAddr = ADDR_FLASH_SECTOR_11;
  int32_t i = 0;
