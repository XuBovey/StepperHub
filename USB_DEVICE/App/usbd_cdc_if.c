...
void usbTx(uint8_t *str,uint16_t len)
{
    uint16_t j = 0;
    if(len < USB_SIZ_STRING_SERIAL)
    {
        usbSendData(str,len);
    }
    else
    {
        for (j = 0;j < len/USB_SIZ_STRING_SERIAL; j++)
        {
            usbSendData((str + USB_SIZ_STRING_SERIAL*j), USB_SIZ_STRING_SERIAL);
        }
        usbSendData((str + USB_SIZ_STRING_SERIAL*j),len%USB_SIZ_STRING_SERIAL);
    }
}
...
void usb_printf(const char *format, ...)
{
    ...
    if(HAL_OK != CDC_Transmit_FS(UserTxBufferFS, length))
    {
